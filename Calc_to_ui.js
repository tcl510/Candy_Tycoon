...
//change to true for a mega ton of money and free purchase, saving is disabled.
const cheat = false;
...
let create_candy_list = () => {
    return [
        new candy(`Raisans`, `Shop`, 4.2, 0,
            new factory(15, 1.12, 0.2, `Shop`), `g`,
            new optimisation(500, 1.3),
            new optimisation(6500, 1.5)
        ),
        new candy(`Suger Kanes`, `Shop`, 6, 2000,
            new factory(1150, 1.15, 0.3, `Shop`), `g`,
            new optimisation(5000, 1.5),
            new optimisation(25000, 1.5)

        ),
        new candy(`Jelly Stomach`, `Shop`, 10, 7000,
            new factory(1000, 1.2, 0.5, `Shop`), `g`,
            new optimisation(40000, 1.5),
            new optimisation(130000, 1.5)

        ),
        new candy(`Tootsie Cynliders`, `Shop`, 15, 20000,
            new factory(2500, 1.21, 0.4, `Shop`), `g`,
            new optimisation(75000, 1.6),
            new optimisation(190000, 1.5)

        ),
        new candy(`m&ns`, `Shop`, 20, 40000,
            new factory(5000, 1.22, 0.5, `Shop`), `g`,
            new optimisation(80000, 1.5),
            new optimisation(120000, 1.5)

        ),
        new candy(`Mantos`, `Shop`, 30, 75000,
            new factory(10000, 1.23, 0.5, `Shop`), `g`,
            new optimisation(145000, 1.5),
            new optimisation(280000, 1.45)

        ),
        new candy(`Skettles`, `Shop`, 40, 90000,
            new factory(20000, 1.24, 0.4, `Shop`), `g`,
            new optimisation(190000, 1.5),
            new optimisation(550000, 1.7)

        ),
        new candy(`Chuppa chups`, `Shop`, 50, 120000,
            new factory(30000, 1.25, 0.5, `Shop`), `g`,
            new optimisation(210000, 1.5),
            new optimisation(750000, 1.7)

        ),
        new candy(`Snackers`, `Shop`, 60, 180000,
            new factory(40000, 1.26, 0.4, `Shop`), `g`,
            new optimisation(285000, 1.5),
            new optimisation(1000000, 1.6)

        ),
        new candy(`Pac man Dots`, `Shop`, 70, 250000,
            new factory(50000, 1.27, 0.25, `Shop`), `g`,
            new optimisation(350000, 1.3),
            new optimisation(1500000, 1.8)

        ),
        new candy(`Hairybro gumies`, `Shop`, 240, 5500000,
            new factory(700000, 1.28, 0.16, `Shop`), `g`,
            new optimisation(14500000, 1.5),
            new optimisation(28000000, 1.5)

        ),
        new candy(`Square Sweets that look round`, `Shop`, 666.67, 15000000,
            new factory(2500000, 1.23, 0.08, `Shop`), `g`,
            new optimisation(45000000, 1.4),
            new optimisation(75000000, 1.7)

        ),
        new candy(`Chocolate River`, `Shop`, 3400, 95000000,
            new factory(5000000, 1.3, 0.04, `Shop`), `g`,
            new optimisation(78500000, 1.5),
            new optimisation(275000000, 1.6)

        ),
        new candy(`Warming toffies`, `Shop`, 11250, 465000000,
            new factory(12000000, 1.31, 0.02, `Shop`), `g`,
            new optimisation(160000000, 1.6),
            new optimisation(575000000, 1.7)

        ),
        new candy(`Everlasting gobstoppers`, `Shop`, 63250, 1200000000,
            new factory(35000000, 1.31, 0.015, `Shop`), `g`,
            new optimisation(575000000, 1.6),
            new optimisation(2012500000, 1.5)

        ),
        new candy(`Wonka bar`, `Shop`, 270000, 4840000000,
            new factory(75000000, 1.32, 0.01, `Shop`), `g`,
...
angular.module("Candy_tycoon", [])
    ...
    .controller("get_names", ['$scope', '$document', '$window', '$sce', '$interval', '$timeout', '$animate', function($scope, $document, $window, $sce, $interval, $timeout, $animate) {
        ...
        $scope.time = time;
        $scope.money = cheat? 100000000000000: 1000;
        ...
        $scope.chart_update = function() {
            ...
            for (i = 0; i < $scope.candies.length; i++) {
                $scope.dps[i] = {
                    name: $scope.candies[i].name,
                    y: $scope.candies[i].total_sold,
                };
            };
            //
            ...
            // $scope.dps = $scope.candies.filter(function(candy) {
            //     if (candy.visibility) {
            //         return candy;
            //     }
            // }).map(a => {
            ...
            // much cleaner method that somehow doesnt work
        ...
        function readFromCookie() {
            ...
            try {
                if (localStorage.getItem("profile_list") !== null) {
                    let profile_list = JSON.parse(localStorage.getItem("profile_list"));
                    try {
                        if (localStorage.getItem("active_profile") !== null) {
                            let active_profile = JSON.parse(localStorage.getItem("active_profile"));
                            try {
                                if (profile_list[active_profile - 1].save !== null) {
                                    let save_data = profile_list[active_profile - 1].save;
                                    ...
                                    $scope.name = profile_list[active_profile - 1].name;
                                    console.log("Loaded");
                                } else {
        ...
        function writeToCookie() {
            ...
            try{
                if (localStorage.getItem("profile_list") !== null) {
                    let profile_list = JSON.parse(localStorage.getItem("profile_list"));
                    profile_list[localStorage.getItem("active_profile") -1].save = save_data;
